...
4. **Variant:** Change the exploration step count from 20 to 5 and to 50 steps per iteration. How does this affect the rate of archive growth and time to goal discovery? Is there a trade-off between breadth and depth of exploration?
5. **Debug:** A Go-Explore implementation never grows the archive beyond the starting state. The code checks `if new_state not in archive` but uses a floating-point tuple from a continuous env as the key, causing near-duplicate states to fail equality checks. Describe the fix (e.g. discretization or hashing) and why exact equality is fragile for continuous states.
6. **Conceptual:** Go-Explore separates *exploration* (finding new states) from *exploitation* (learning to robustly reach them). Why is this separation beneficial? What failure mode of curiosity-driven methods does it avoid?
