...
4. **Variant:** Change the number of inner adaptation steps from 1 to 5. Does more adaptation steps improve held-out task performance, or does it lead to overfitting to the adaptation data?
5. **Debug:** Meta-training loss decreases but held-out task performance after adaptation stays at random. The inner update uses the *meta* learning rate (1e-3) instead of a separate *inner* learning rate (e.g. 0.1). Explain why a too-small inner learning rate prevents adaptation and how to set the two learning rates independently.
6. **Conceptual:** What does the meta-learner actually learn — task-specific knowledge or task-agnostic structure? Describe in one paragraph what "good initial parameters" means in the context of MAML and why this is different from simply pretraining on a large mixed dataset.
