...
4. **Variant:** Replace the PAIRED adversary with a random curriculum (uniformly random mazes) and a fixed curriculum (hand-designed easy-to-hard sequence). Compare agent generalization on a held-out test set across all three curricula after 500 iterations.
5. **Debug:** The PAIRED adversary immediately generates impossible mazes (no path from start to goal) and the agent always gets 0 reward. The adversary is maximizing agent regret but never receives a penalty for unsolvable environments. Add a solvability check and describe how to penalize the adversary for generating unsolvable mazes.
6. **Conceptual:** PAIRED optimizes for the "regret" of the agent — the gap between a protagonist's return and an antagonist's return. Why is regret a better curriculum signal than simply minimizing the agent's return? What prevents the adversary from always generating trivially hard environments?
