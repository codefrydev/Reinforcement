...
4. **Variant:** Change the number of meta-episodes per trial (the RNN's context length) from 3 to 10. Does a longer context let the RNN identify the task more reliably? Plot adaptation return vs context length.
5. **Debug:** The RL² agent always takes the same action regardless of the task. Hidden state logging shows the RNN hidden state is near zero throughout. The bug is that `done` signals reset the hidden state to zero mid-trajectory. Explain why resetting within a meta-episode destroys the context and where the reset should actually occur.
6. **Conceptual:** RL² "meta-learns" through gradient descent over many tasks, but adapts at test time through recurrence only (no gradient steps). What are the advantages of this compared to MAML? What is the main limitation of the recurrent approach?
