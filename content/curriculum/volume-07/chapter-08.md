...
4. **Variant:** Change the inner learning rate from 0.01 to 0.5. How does the choice of inner lr affect adaptation speed (return after 1 inner step) and meta-training stability? Is there a value where meta-training diverges?
5. **Debug:** MAML trains but the meta-gradient is always zero. The inner update was implemented with `torch.no_grad()`, preventing gradients from flowing through the inner step. Explain why removing `no_grad()` is required and how to verify that second-order gradients are flowing.
6. **Conceptual:** MAML and fine-tuning both adapt a pretrained model to a new task. What is the key difference in *how they train* the initial parameters? Why might MAML's initial parameters generalize better with fewer gradient steps than a naively pretrained model?
