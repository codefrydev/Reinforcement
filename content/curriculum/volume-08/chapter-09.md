...
4. **Variant:** Vary the offline pretraining dataset size: 10k, 50k, 200k transitions. How does the amount of offline data affect the quality and stability of the subsequent online finetuning?
5. **Debug:** Offline-to-online finetuning shows a "performance dip" in the first 10k online steps — the agent gets worse before improving. The Q-values from offline training are overestimates for actions the online policy now explores. Describe why the value function "corrects" itself during online finetuning and what conservative Q initialization would prevent the dip.
6. **Conceptual:** Offline pretraining followed by online finetuning is similar in spirit to large-model pretraining then task-specific finetuning in supervised learning. Describe one key difference: why does the distribution shift between offline data and the online environment make RL finetuning more challenging than supervised finetuning?
