...
4. **Variant:** Use the same CQL code on a *mixed* dataset: 50% expert, 50% random transitions. Does CQL still outperform naive SAC? How does dataset composition change the optimal α?
5. **Debug:** CQL training is stable but evaluation return is worse than behavioral cloning. Logging shows α=10, causing Q-values to be uniformly near zero. The penalty is so strong that the Q-function can no longer distinguish good from bad actions. Describe how to tune α adaptively (e.g. keep Q on in-distribution actions ≥ some threshold).
6. **Conceptual:** CQL adds a penalty on Q(s, a) for out-of-distribution actions. Explain intuitively why this is equivalent to learning a lower bound on the true Q-function. When would a lower bound be too conservative and hurt performance?
