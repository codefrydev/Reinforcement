...
4. **Variant:** Vary the number of DAgger rounds from 1 to 10. Does performance plateau after a certain number of rounds? What factor (dataset size or distribution shift coverage) limits DAgger's improvement?
5. **Debug:** A DAgger implementation runs 5 rounds but performance does not improve beyond round 1. The new states collected by the student policy are added to the buffer, but the model is only retrained on the *new* data each round (not on the full aggregated dataset). Fix the aggregation step and explain why training on the full dataset is essential to DAgger's convergence guarantee.
6. **Conceptual:** DAgger requires querying the expert at states the student visits. In real-world robotics, this may be expensive or dangerous. Describe two practical strategies that reduce the number of expert queries while still correcting the most critical distributional errors.
