...
4. **Variant:** Train BC with only 5 expert episodes, then 20, then 100. Plot evaluation return vs number of demonstrations. At what point does more data stop helping and why?
5. **Debug:** A BC policy achieves high action prediction accuracy (>90%) on the held-out set but has near-zero evaluation return. The expert dataset was collected with observation noise added, but the evaluation environment has no noise. Explain why this mismatch causes the compounding error problem and how DAgger addresses it.
6. **Conceptual:** Behavioral cloning minimizes the imitation gap in terms of action prediction error. Explain why a small action prediction error can still compound into a large trajectory divergence. Use the concept of "distribution shift" to frame your answer.
