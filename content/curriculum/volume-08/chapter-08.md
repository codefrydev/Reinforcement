...
{{< chart type="line" palette="return" title="Task return and style score (AMP)" labels="0, 500, 1000, 1500, 2000" data="0, 200, 500, 800, 1200" xLabel="Step" yLabel="Return" >}}
...
4. **Variant:** Use two different reference motion styles (e.g. walking vs running clips). Train a single policy conditioned on a style label. Does the policy learn to interpolate between styles, and does task return remain high for both?
5. **Debug:** The style discriminator in an AMP implementation converges to 50% accuracy immediately (cannot distinguish reference from agent motions). The reference transitions are sampled once at the start and never refreshed; as the agent improves, real agent motions now look like the reference. Describe how to properly maintain and sample the reference motion buffer throughout training.
6. **Conceptual:** Physics-based motion imitation (AMP-style) is harder to train than kinematic imitation (just replaying joint angles). What additional challenges does the physics simulation introduce, and why is a style discriminator more robust to these challenges than an explicit pose-matching loss?
