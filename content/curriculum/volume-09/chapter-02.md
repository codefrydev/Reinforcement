...
4. **Variant:** Change the Prisoner's Dilemma payoffs to a Stag Hunt (cooperation is individually risky but jointly optimal). How many Nash equilibria exist, and which one do two independent Q-learners converge to? Does initialization affect which equilibrium is reached?
5. **Debug:** Two Q-learning agents playing Matching Pennies alternate between (H,H) and (T,T) but never converge. The learning rate α=0.5 is too high, causing Q-values to oscillate. Describe how to use a decaying learning rate schedule and what convergence criterion you would use for a cyclic game.
6. **Conceptual:** A Nash equilibrium is a fixed point where no agent wants to deviate *given the other's strategy*. Why is this a fixed point rather than an optimum? Describe a game where the Nash equilibrium is Pareto-suboptimal — both agents could be better off, but neither would unilaterally deviate.
