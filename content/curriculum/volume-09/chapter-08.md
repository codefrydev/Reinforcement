...
4. **Variant:** Change from homogeneous agents (all same role) to heterogeneous agents (two defenders and one attacker with different action sets). Does parameter sharing still help, or does it hurt by forcing different-role agents to share weights?
5. **Debug:** MAPPO training shows the centralized value function V(s) converging well, but policy gradient updates are very noisy. The advantage estimates A = R - V(s) are computed using global state s in V but local observation o in the policy, causing a mismatch in the baseline. Explain the correct way to compute advantages with a centralized critic and decentralized actor.
6. **Conceptual:** MAPPO uses a centralized value function during training to reduce variance. Explain why reducing variance in the advantage estimates is especially important in multi-agent settings compared to single-agent PPO. What grows with the number of agents that makes policy gradient variance worse?
