...
4. **Variant:** Change from 2 to 4 agents in the meet-up task. Does VDN's additive decomposition scale, or does the gap between VDN and true joint Q grow with more agents and more complex coordination requirements?
5. **Debug:** A VDN implementation converges but team return is lower than a centralized controller baseline. Logging shows Q_tot is computed as `Q_tot = Q_1 + Q_2`, but during the Bellman target computation the individual targets are clipped, causing the sum to not satisfy the additive decomposition. Describe how to compute the correct VDN Bellman target without per-agent clipping.
6. **Conceptual:** VDN decomposes the joint value function additively. State the IGM (Individual-Global-Max) property and explain why it is necessary for decentralized execution. Does VDN satisfy IGM? What class of cooperative tasks can it represent exactly?
