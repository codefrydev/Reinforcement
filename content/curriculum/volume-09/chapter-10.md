...
{{< chart type="bar" palette="comparison" title="Return (coordination task)" labels="No comm, With comm" data="40, 95" yLabel="Return" >}}
...
4. **Variant:** Limit the communication bandwidth: instead of a 2D continuous message, allow only 1 bit per step. How much does the task success rate drop? Is the swap task solvable with only 1 bit of communication per timestep?
