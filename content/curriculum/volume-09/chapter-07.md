...
4. **Variant:** Use the non-additive synergy task from Chapter 86's challenge. Confirm QMIX outperforms VDN there. Now design a task that violates QMIX's monotonicity constraint — what shape of joint Q function would require non-monotonic mixing?
5. **Debug:** A QMIX implementation violates the monotonicity constraint: the mixing network produces negative weights because the absolute value / softplus activation was omitted. The result is that decentralized greedy actions no longer maximize Q_tot. Add the positivity constraint and verify by checking that all mixing weights ≥ 0 for a batch of states.
6. **Conceptual:** QMIX enforces a monotonic relationship between individual Q_i and Q_tot, which limits the class of joint Q functions it can represent. Explain why this is not a problem in many cooperative tasks (i.e., why monotonic factorizations are sufficient in practice), and describe the specific multi-agent coordination pattern that would break QMIX.
