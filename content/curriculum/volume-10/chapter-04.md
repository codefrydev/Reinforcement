...
4. **Variant:** Add a user fatigue model: P(click) decreases by 10% for each consecutive recommendation from the same category. Does the RL policy adapt to avoid fatigue (explore across categories) compared to the greedy baseline?
5. **Debug:** The recommender RL agent learns to show the same popular item repeatedly because it has the highest P(click) and the episode is short. The reward is purely click-based with no penalty for repetition. Implement an explicit repetition penalty and describe why short-horizon greedy recommendation is harmful for long-term user satisfaction.
6. **Conceptual:** Maximizing engagement metrics (clicks, watch time) can lead to "filter bubbles" and user polarization. Explain why RL-based recommendation systems are particularly susceptible to this failure mode compared to supervised collaborative filtering, and describe one constraint or objective modification that partially addresses it.
