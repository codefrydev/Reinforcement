...
4. **Variant:** Increase the domain randomization range to ±50% for mass. Does the policy become more robust, or does overly wide randomization hurt performance even on the nominal environment? Find the range that maximizes robustness without sacrificing nominal performance.
5. **Debug:** A sim-to-real policy fails immediately in the real world despite near-perfect sim performance. Analysis shows the policy relies on precise torque measurements that the real robot's sensors quantize coarsely. Describe how to add sensor noise and actuator delay to the simulation to close this gap.
6. **Conceptual:** Sim-to-real transfer fails when the "reality gap" is large. Name three sources of reality gap (beyond physics parameters) that domain randomization alone cannot solve, and suggest a mitigation for each.
