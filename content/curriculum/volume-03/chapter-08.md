...
4. **Variant:** Try \\(\\alpha \\in \\{0.4, 0.6, 0.8\\}\\) for PER priority sharpness. Higher \\(\\alpha\\) means more aggressive prioritization. Does very high \\(\\alpha\\) improve or hurt sample efficiency on CartPole?
5. **Debug:** The code below forgets to add \\(\\epsilon\\) to the priority, causing zero-priority transitions that can never be sampled. Fix it.

{{< pyrepl code="import random\n\nclass SimplePER:\n    def __init__(self, capacity=1000, alpha=0.6):\n        self.buffer = []\n        self.priorities = []\n        self.alpha = alpha\n\n    def add(self, transition, td_error):\n        # BUG: priority can be zero if td_error == 0\n        priority = abs(td_error) ** self.alpha  # missing epsilon\n        self.buffer.append(transition)\n        self.priorities.append(priority)\n\n    def sample(self, n):\n        probs = [p / sum(self.priorities) for p in self.priorities]\n        return random.choices(self.buffer, weights=probs, k=n)\n\n# Fix: use abs(td_error) + 1e-6 before raising to alpha\nbuf = SimplePER()\nbuf.add('transition_1', 0.0)  # would get priority 0 — bug!\nprint('Priority of zero-error transition:', buf.priorities[-1])" height="240" >}}

6. **Conceptual:** Why do we anneal \\(\\beta\\) (the IS weight exponent) from a small value to 1 over training? What would happen if we used \\(\\beta=1\\) from the start?
7. **Recall:** State the importance-sampling weight formula \\(w_i = \\ldots\\) from memory and explain what \\(N\\), \\(P(i)\\), and \\(\\beta\\) represent.
